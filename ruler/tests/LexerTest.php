<?php
	...
	final class LexerTest extends TestCase{
		...
		public function testIdentifiers(): void{
			$tokens = $this->lexer->tokenize('one TWO _one_TWO');
			$this->assertEquals(3, count($tokens));
			$this->assertInstanceOf(Identifier::class, $tokens[0]);
			$this->assertInstanceOf(Identifier::class, $tokens[1]);
			$this->assertInstanceOf(Identifier::class, $tokens[2]);
			$this->assertEquals('one', $tokens[0]->getValue());
			$this->assertEquals('TWO', $tokens[1]->getValue());
			$this->assertEquals('_one_TWO', $tokens[2]->getValue());
		}

		public function testSimpleTokens(): void{
			$tokens = $this->lexer->tokenize('1.2.3.4 "token');
			$this->assertEquals(2, count($tokens));
			$this->assertInstanceOf(Token::class, $tokens[0]);
			$this->assertInstanceOf(Token::class, $tokens[1]);
			$this->assertEquals('1.2.3.4', $tokens[0]->getValue());
			$this->assertEquals('"token', $tokens[1]->getValue());
		}
